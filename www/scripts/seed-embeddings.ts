...
async function main() {
  ...
  console.log(`Found ${facilitiesToEmbed.length} facilities without embeddings.`);
  ...
  for (let i = 0; i < facilitiesToEmbed.length; i += batchSize) {
    ...
    await Promise.all(batch.map(async (facility) => {
      try {
        ...
        const parts = [
          ...
          facility.specialtiesRaw ? `Specialties: ${facility.specialtiesRaw}` : '',
          facility.proceduresRaw ? `Procedures: ${facility.proceduresRaw}` : '',
          facility.equipmentRaw ? `Equipment: ${facility.equipmentRaw}` : '',
          facility.capabilitiesRaw ? `Capabilities: ${facility.capabilitiesRaw}` : '',
    ...
    console.log(`Processed ${Math.min(i + batchSize, facilitiesToEmbed.length)}/${facilitiesToEmbed.length} (Success: ${successCount}, Errors: ${errorCount})`);
  }
