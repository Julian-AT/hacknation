...
const CSV_PATH = path.resolve(
  __dirname,
  "../../assets/data/ghana-facilities.csv"
);
...
function normalizeRegion(
  ...
  if (city) {
    ...
    if (cityLower === "accra" || cityLower === "tema") {
    ...
    if (cityLower === "takoradi" || cityLower === "sekondi-takoradi") {
      return "Western";
    }
    if (cityLower === "cape coast") {
      return "Central";
    }
    if (cityLower === "ho") {
      return "Volta";
    }
    if (cityLower === "koforidua") {
      return "Eastern";
    }
    if (cityLower === "sunyani") {
      return "Bono"; // Formerly Brong-Ahafo
    }
    if (cityLower === "wa") {
      return "Upper West";
    }
    if (cityLower === "bolgatanga") {
      return "Upper East";
    }
  }
  return null;
...
function getCoordinates(
  ...
  if (city === "Sekondi-Takoradi") {
    return CITY_COORDS.Takoradi;
  }
...
async function main() {
  ...
  for (const record of records) {
    ...
    // Parse integers
    const numDoctors = record.numberDoctors
      ? Number.parseInt(record.numberDoctors, 10)
      : null;
    const capacity = record.capacity
      ? Number.parseInt(record.capacity, 10)
      : null;
    const areaSqm = record.area ? Number.parseInt(record.area, 10) : null;
    const yearEstablished = record.yearEstablished
      ? Number.parseInt(record.yearEstablished, 10)
      : null;
    const pkUniqueId = record.pk_unique_id
      ? Number.parseInt(record.pk_unique_id, 10)
      : null;
    ...
    facilitiesData.push({
      ...
      lat,
      lng,
      numDoctors: Number.isNaN(numDoctors) ? null : numDoctors,
      capacity: Number.isNaN(capacity) ? null : capacity,
      areaSqm: Number.isNaN(areaSqm) ? null : areaSqm,
      yearEstablished: Number.isNaN(yearEstablished) ? null : yearEstablished,
