...
import { and, isNotNull, ilike, eq, sql, desc, cosineDistance } from "drizzle-orm";
import { tool } from "ai";
...
const EVIDENCE_CATEGORIES = {
  temporary_equipment: {
    ...
    description: "Equipment that appears to be brought in periodically rather than permanently installed (Q3.2)",
    patterns: [
      "brought in", "borrowed", "shared equipment", "loaned",
      "mobile unit", "portable", "temporary", "on loan",
      "periodic", "intermittent", "seasonal", "donated temporarily",
      "equipment rotation", "shared with", "comes with",
    ],
  },
  surgical_camp: {
    label: "Surgical Camp / Temporary Mission Evidence",
    description: "Evidence of surgical camps or temporary medical missions rather than standing services (Q6.5, Q8.4)",
    patterns: [
      "surgical camp", "eye camp", "medical camp", "health camp",
      "screening camp", "dental camp", "medical mission",
      "outreach program", "community outreach", "field surgery",
      "ngo", "volunteer team", "international team", "mission trip",
      "charity", "free surgery", "sponsored", "funded by",
      "remote surgery", "humanitarian", "relief",
      "pop-up", "temporary clinic", "field hospital",
    ],
  },
  individual_tied: {
    label: "Individual-Tied Services",
    description: "Services that depend on specific individuals rather than institutions, implying fragile continuity (Q6.6)",
    patterns: [
      "dr.", "dr ", "professor ", "prof.", "prof ",
      "named surgeon", "the surgeon", "our surgeon",
      "sole practitioner", "one specialist", "single specialist",
      "only doctor", "only surgeon", "personal practice",
      "private practice of", "led by", "performed by",
      "consultant surgeon", "visiting consultant",
    ],
  },
  equipment_age: {
    label: "Equipment Age / Legacy Signals",
    description: "Indicators of older/legacy equipment vs newer modalities (Q7.4)",
    patterns: [
      // Legacy signals
      "analog", "film-based", "manual", "outdated",
      "old model", "refurbished", "second-hand", "secondhand",
      "donated equipment", "reconditioned", "legacy",
      // Modern signals
      "digital", "latest", "state-of-the-art", "new",
      "recently acquired", "upgraded", "modern",
      "latest generation", "brand new", "newly installed",
    ],
    ...
    subCategories: {
      ...
      modern: [
        "digital", "latest", "state-of-the-art",
        "recently acquired", "upgraded", "modern",
        "latest generation", "brand new", "newly installed",
      ],
    },
  ...
  ngo_substitution: {
    ...
    description: "Evidence that periodic NGO activity substitutes for permanent capacity (Q8.4)",
    patterns: [
      "ngo", "non-governmental", "international organization",
      "medical mission", "volunteer doctor", "visiting team",
      "outreach event", "annual campaign", "periodic screening",
      "sponsored by", "funded by", "supported by",
      "partnership with ngo", "collaboration with ngo",
      "charity organization", "humanitarian organization",
      "international aid", "development partner",
      "remote surgery day", "free surgery day",
    ],
  },
} as const;
...
function extractSnippets(text: string, patterns: string[], maxSnippets = 3): string[] {
...
export const analyzeTextEvidence = tool({
  ...
  inputSchema: z.object({
    searchQuery: z
      ...
      .max(500)
      .optional()
      .describe("Semantic search query to find relevant facilities (e.g., 'ophthalmology', 'surgery equipment'). If omitted, analyzes all facilities matching the filters."),
    facilityId: z
      .number()
      .int()
      .positive()
  ...
  execute: async ({ searchQuery, facilityId, region, evidenceCategory, limit }, { abortSignal }) => {
    ...
    try {
      ...
      if (facilityId) {
        ...
        const queryEmbedding = await withTimeout(embed(searchQuery), 10_000, abortSignal);
        ...
        for (const catKey of categoriesToCheck) {
          ...
          if (matched.length > 0) {
            ...
            if (catKey === "equipment_age") {
              ...
              const legacyMatches = subCats.legacy.filter((p) => allText.includes(p));
              const modernMatches = subCats.modern.filter((p) => allText.includes(p));
      ...
      log.error(error, { searchQuery, facilityId, region, evidenceCategory }, Date.now() - start);
